...
const heroSlides = [
  { id: 1, image: heroImg1, title: 'Artisan Heritage',    subtitle: 'Handcrafted with Love',               link: '/shop?category=Bangles'  },
  { id: 2, image: heroImg2, title: 'Royal Bangles',       subtitle: 'Elegance in Every Detail',            link: '/shop?category=Bangles'  },
  { id: 3, image: heroImg5, title: 'Modern Earrings',     subtitle: 'Timeless Beauty',                     link: '/shop?category=Earrings' },
  { id: 4, image: heroImg3, title: 'Bridal Bangles',      subtitle: 'Your Special Day, Our Special Touch', link: '/shop?category=Bangles'  },
  { id: 5, image: heroImg6, title: 'Traditional Jhumkas', subtitle: 'Celebrate with SAKSAAS',              link: '/shop?category=Earrings' },
...
export const Home = () => {
  ...
  if (loading) return <div className="h-screen flex items-center justify-center">Loading SAKSAAS...</div>;
    ...
    <div className="space-y-20 pb-20">
      ...
      <section className="relative h-[80vh] overflow-hidden">
        <AnimatePresence mode="wait">
          <motion.div key={currentSlide} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 1 }} className="absolute inset-0">
            <img src={heroSlides[currentSlide].image} alt={heroSlides[currentSlide].title} loading="lazy" className="w-full h-full object-cover" />
            <div className="absolute inset-0 bg-black/30 flex items-center justify-center text-center">
              <div className="container mx-auto px-4">
                <motion.p initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }} className="text-white uppercase tracking-[0.3em] text-sm mb-4 font-medium">
                  {heroSlides[currentSlide].subtitle}
                </motion.p>
                <motion.h1 initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.4 }} className="text-white text-5xl md:text-7xl font-serif font-bold mb-8">
                  {heroSlides[currentSlide].title}
                </motion.h1>
                <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.6 }}>
                  <Link to={heroSlides[currentSlide].link} className="inline-block bg-white text-stone-900 px-8 py-3 rounded-full font-medium hover:bg-brand-deep-green hover:text-white transition-all duration-300">
                    Explore Collection
                  </Link>
                </motion.div>
              </div>
        ...
        <button onClick={prevSlide} className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/20 backdrop-blur-md text-white flex items-center justify-center hover:bg-white hover:text-stone-900 transition-all">
          <ChevronLeft size={24} />
        </button>
        <button onClick={nextSlide} className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/20 backdrop-blur-md text-white flex items-center justify-center hover:bg-white hover:text-stone-900 transition-all">
      ...
      <section className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Link to="/shop?category=Earrings" className="group relative aspect-square overflow-hidden rounded-2xl shadow-md">
            <img src={catEarringsImg} alt="Earrings" loading="lazy" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
            <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent flex items-end justify-center p-6">
              <h3 className="text-white font-serif text-2xl font-bold">Earrings</h3>
            </div>
          </Link>
          <Link to="/shop?category=Bangles" className="group relative aspect-square overflow-hidden rounded-2xl shadow-md">
            <img src={catBanglesImg} alt="Bangles" loading="lazy" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
            <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent flex items-end justify-center p-6">
              <h3 className="text-white font-serif text-2xl font-bold">Bangles</h3>
            </div>
          </Link>
        </div>
      </section>
      ...
      <section className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            { img: prodImg1, alt: 'Modern Earrings',      link: '/shop?category=Earrings&subCategory=Modern Jhumkas'      },
            { img: prodImg2, alt: 'Silk Thread Jhumkas',  link: '/shop?category=Earrings&subCategory=Silk Thread Jhumkas' },
            { img: prodImg3, alt: 'Kundan Bangles',       link: '/shop?category=Bangles&subCategory=Traditional'          },
            { img: prodImg4, alt: 'Kundan Jhumkas',       link: '/shop?category=Earrings&subCategory=Jhumkas'             },
            { img: prodImg5, alt: 'Premium Bangles',      link: '/shop?category=Bangles&subCategory=Bridal'               },
            { img: prodImg6, alt: 'Silk Thread Jhumkas',  link: '/shop?category=Earrings&subCategory=Silk Thread Jhumkas' },
          ].map((item, i) => (
            <Link key={i} to={item.link} className="aspect-square relative group overflow-hidden rounded-lg shadow-md hover:shadow-lg transition-shadow">
              <img src={item.img} alt={item.alt} loading="lazy" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
      ...
      <section className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
          {products.filter(p => p.newArrival).slice(0, 6).map(product => (
            <ProductCard key={product.id} product={product} />
          ))}
        </div>
      </section>
      ...
      <section className="container mx-auto px-4">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch">
          <div className="lg:col-span-1 relative rounded-2xl overflow-hidden min-h-[400px]">
            <img src={budgetBannerImg} alt="Budget Friendly" loading="lazy" className="absolute inset-0 w-full h-full object-cover" />
            <div className="absolute inset-0 bg-brand-deep-green/40 flex flex-col justify-center p-10 text-white">
              ...
              <Link to="/shop?filter=under500" className="inline-block bg-white text-brand-deep-green px-8 py-3 rounded-full font-bold w-fit hover:bg-stone-900 hover:text-white transition-all">
                Shop Now
              </Link>
            </div>
          </div>
          <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-8">
            {products.filter(p => p.under500).slice(0, 6).map(product => (
              <ProductCard key={product.id} product={product} />
            ))}
          </div>
        </div>
      ...
      <section className="bg-stone-900 text-white py-20">
        <div className="container mx-auto px-4 text-center max-w-3xl">
          <img src={storyBannerImg} alt="Story" loading="lazy" className="w-24 h-24 rounded-full mx-auto mb-6 object-cover border-2 border-brand-gold" />
          ...
          <Link to="/about" className="inline-block border-2 border-brand-gold text-brand-gold px-10 py-3 rounded-full font-bold hover:bg-brand-gold hover:text-white transition-all">
            Read Our Story
          </Link>
        </div>
      </section>
